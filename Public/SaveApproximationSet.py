...
def saveApproximationSet(A, ppf, scheme, problem, distance, run, N, m, M, sample_size=None, p_minkowski=None, mode='save_all'):
    ...
    if M is not None:
        file_name = 'Results/Approximations/'+ppf+'_'+scheme+'_'+problem+'_'+distance+'_{0:0=2d}D'.format(m)+'_{0:0=4d}N'.format(N)+'_{0:0=2d}M'.format(M)+'_R{0:0=2d}'.format(run)
        f = open(file_name+'.pof', 'w')
        ...
        if distance != "minkowski":
            ...
            file_name = prefix+'_'+distance+'_{0:.3f}_{1:0=2d}D'.format(p_minkowski, m)+'_{0:0=4d}N'.format(N)+'_R{0:0=2d}'.format(run)+'.pof' 
